...
phase2 = ROOT / "backend/phase2_app.py"
phase2_text = phase2.read_text(encoding="utf-8")
if "from phase3_routes import register as register_phase3" not in phase2_text:
    phase2_text = phase2_text.replace(
        "from phase2_routes import register as register_phase2\n",
        "from phase2_routes import register as register_phase2\nfrom phase3_routes import register as register_phase3\n",
        1,
    )
if "register_phase3(app)" not in phase2_text:
    phase2_text = phase2_text.replace(
        "register_phase2(app)\n",
        "register_phase2(app)\nregister_phase3(app)\n",
        1,
...
app = ROOT / "frontend/src/AppPhase6.jsx"
...
if old_tools not in text:
    raise RuntimeError("Expected STUDIO_TOOLS block was not found")
text = text.replace(old_tools, new_tools, 1)

start = text.index("function StudioPanel(")
end = text.index("\n\nexport default function AppPhase6", start)
replacement = r'''function StudioPanel({ close, tool, setTool, activeId, sources, activeSources, userId, openSources }) {
