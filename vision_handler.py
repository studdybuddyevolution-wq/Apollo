...
def render_image_question_widget(groq_key: str, key_suffix: str = "main", on_answered=None, compact: bool = False):
  ...
  compact: if True, renders inside a small popover (camera icon trigger)
    instead of an always-visible expander row.
  """
  container = st.popover("📷", use_container_width=False) if compact else st.expander(
      "📷 Ask With a Photo (handwriting, diagrams, textbook pages)", expanded=False
  )
  with container:
...
def render_homework_grading_widget(groq_key: str, key_suffix: str = "main", on_answered=None, compact: bool = False):
  ...
  compact: if True, renders inside a small popover (checklist icon trigger)
    instead of an always-visible expander row."""
  container = st.popover("🧾", use_container_width=False) if compact else st.expander(
      "🧾 Grade Homework From a Photo", expanded=False
  )
  with container:
