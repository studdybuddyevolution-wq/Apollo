...
def _extract_label_words(kind: str, code: str) -> set[str]:
    """Best-effort extraction of the actual text labels a diagram displays,
    for both SVG (<text> contents) and Mermaid (node/edge labels, with
    directive/keyword lines and arrow syntax stripped out). This is a
    heuristic, not a guarantee -- it exists to catch obvious invented
    content, not to formally verify correctness."""
    if kind == "svg":
...
def content_overlap_ratio(kind: str, code: str, student_content: str) -> float:
    """Fraction of the diagram's own label words that actually appear
    somewhere in the student's original content. Low overlap suggests the
    model invented labels/ideas not present in what the student wrote."""
