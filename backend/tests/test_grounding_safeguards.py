...
def test_study_report_retries_when_overlap_is_weak(monkeypatch):
    ...
    monkeypatch.setattr(phase3_routes, "_context_or_400", lambda *args, **kwargs: (context, ["biology.md"]))
    monkeypatch.setattr(
        phase3_routes,
        "_safe_generation",
...
@pytest.mark.parametrize(
    ("answer", "expected"),
    [
        ("DNA is the genetic material in cells. It stores genetic information in cells.", True),
        ("Quantum mechanics describes a wavefunction for physical systems.", False),
    ],
)
def test_deep_search_emits_grounding_check(monkeypatch, answer, expected):
    plan = {
        "topic": "DNA",
        "outline": ["DNA"],
        "verification": {
    ...
    events = _payloads(main._stream_deep_research(request, "SYSTEM", "", []))
    checks = [event for event in events if event.get("type") == "grounding_check"]
    assert len(checks) == 1
    assert checks[0]["verified"] is expected
    assert bool(checks[0]["warning"]) is (not expected)
