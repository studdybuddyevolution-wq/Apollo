-- Apollo retrieval/processing upgrade.
...
-- pgvector is optional on self-hosted Postgres. Supabase supports it,
-- but the migration must not take Apollo down when the extension is unavailable.
DO $$
BEGIN
  BEGIN
...
BEGIN
  IF EXISTS (SELECT 1 FROM pg_type WHERE typname = 'vector') THEN
    EXECUTE 'ALTER TABLE apollo_chunks ADD COLUMN IF NOT EXISTS embedding vector(768)';
    EXECUTE 'CREATE INDEX IF NOT EXISTS idx_apollo_chunks_embedding ON apollo_chunks USING ivfflat (embedding vector_cosine_ops) WITH (lists = 100)';
  END IF;
END $$;
...
CREATE INDEX IF NOT EXISTS idx_apollo_source_insights_scope
  ON apollo_source_insights(notebook_id, source_name);
