...
def _load_overview_context(notebook_id: str, source_names: list[str], max_chunks: int = 6) -> tuple[str, list[str]]:
    ...
    if not path.exists(): return "", source_names
    try: chunks = json.loads(path.read_text(encoding="utf-8"))
    except Exception: return "", source_names
    allowed = set(source_names or [])
    filtered = [c for c in chunks if not allowed or c.get("source") in allowed]
    if not filtered: return "", source_names
    selected, seen = [], set()
    for c in filtered:
        src = c.get("source", "unknown source")
        if src not in seen:
            selected.append(c); seen.add(src)
            if len(selected) >= max_chunks: break
    selected_ids = {c.get("id") for c in selected}
    for c in filtered:
        if len(selected) >= max_chunks: break
        if c.get("id") not in selected_ids: selected.append(c)
...
def _system_message(request: ChatRequest, context: str, source_names: list[str]) -> dict[str, str]:
    ...
    content = ("You are Apollo Omni AI, a helpful academic AI companion. "
...
def _stream_groq(request: ChatRequest, messages: list[dict[str, str]], model: str):
    ...
    if model.startswith("openai/gpt-oss") or model.startswith("qwen/"):
        ...
        if model.startswith("openai/gpt-oss"): kwargs["reasoning_effort"] = "medium"
...
def _search_tavily(query: str, deep: bool = False) -> tuple[str, list[dict[str, str]]]:
    ...
    response = TavilyClient(api_key=key).search(query=query, search_depth="advanced" if deep else "basic", topic="general", max_results=8 if deep else 5, chunks_per_source=3, include_answer=False, include_raw_content=deep)
    sources, seen, blocks, total = [], set(), [], 0
    max_chars = 15000 if deep else 11000
    for i, item in enumerate(response.get("results", []) or [], 1):
        url = str(item.get("url") or "").strip(); title = str(item.get("title") or url).strip(); text = str(item.get("raw_content") or item.get("content") or "").strip()
        if url and url not in seen: sources.append({"title": title[:180], "url": url}); seen.add(url)
        if not text: continue
        block = f"SOURCE {i}: {title}\nURL: {url}\n{text}"
        remaining = max_chars - total
        if remaining <= 0: break
        block = block[:remaining]; blocks.append(block); total += len(block)
    return "\n\n---\n\n".join(blocks), sources[:8]

def _deep_research_queries(question: str, study: bool) -> list[str]:
    if study:
        return [f"{question} official sources and current evidence; focus on facts that complement or challenge a student's notes", f"{question} authoritative explanation, examples, major dates or definitions, and common misconceptions", f"{question} primary or institutional sources plus recent context and important caveats"]
    return [f"{question} authoritative overview primary sources current evidence", f"{question} detailed explanation examples historical or technical context and important caveats", f"{question} competing perspectives primary sources limitations and facts that would change the conclusion"]
...
def _stream_web_with_tavily(request: ChatRequest, system_content: str):
    ...
    if not question: raise RuntimeError("No user query supplied for web research")
    ...
    dossiers, all_sources = [], []
    for query in queries:
        web_context, sources = _search_tavily(query, deep=deep)
        if web_context: dossiers.append(f"RESEARCH QUERY: {query}\n{web_context}")
        all_sources.extend(sources)
    if not dossiers: raise RuntimeError("Tavily returned no usable web results")
    unique_sources = list({s["url"]: s for s in all_sources if s.get("url")}.values())[:12]
    web_context = "\n\n========== RESEARCH PASS ==========\n\n".join(dossiers)
    if deep:
    ...
    prompt = _conversation_text(request.messages, instruction) + f"\n\nTAVILY RESEARCH DOSSIER:\n{web_context}"
    if unique_sources: yield _event({"type": "sources", "sources": unique_sources})
